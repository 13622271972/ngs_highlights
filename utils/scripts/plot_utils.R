...
# * reduce dataset ----
reduced_play_data <- play_data %>% filter(frame >= final_frame, frame <= final_frame+10)
...
player_data <- reduced_play_data %>% 
  select(frame, homeTeamFlag, teamAbbr, displayName, gsisId, position, position, positionGroup,
         x, y, s, o, dir, event) %>% 
  filter(displayName != "ball")

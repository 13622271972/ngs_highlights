---
title: "Getting Started With NFL Highli
---
...
*This is meant to be a very simple introduction to working with the tracking data housed in this repo. For a more in-depth guide on working with the NFL's tracking data, check out [Mike Lopez's notebook](https://www.kaggle.com/statsbymichaellopez/nfl-tracking-wrangling-voronoi-and-sonars) from the most recent Big Data Bowl.*
...
# *source("D:\\work\\ngs_highlights-master\\ngs_highlights-master\\utils\\scripts\\data_utils.R")
# *source("D:\\work\\ngs_highlights-master\\ngs_highlights-master\\utils\\scripts\\plot_utils.R")
...
```{r fetch_play_data}
play_data <- fetch_play_data(playKey_ = 241)
```
...
play_frames <- plot_field() + # plot_field() is a helper function that returns a ggplot2 object of an NFL field
  ...
  geom_point(
    ...
    shape = 21, alpha = 1, size = 8
  ) +
  # away team jersey numbers
  geom_text(
    data = player_data %>% filter(teamAbbr == a_team),
    mapping = aes(x = x, y = y, label = ifelse(position == "OLB", "MLB", position)),
  ...
  geom_line(
    data = player_data,
    mapping = aes(x = x, y = y, group = gsisId, color = teamAbbr),
    size = 1
  ) +
  geom_text(
    data = player_data %>% filter(teamAbbr == h_team),
...
```{r, echo=F}
anim_save("./play_anim_example.gif", play_anim)
```
